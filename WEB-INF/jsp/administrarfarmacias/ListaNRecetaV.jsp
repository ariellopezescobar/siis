...
<form name=formaRet method=post action='<c:url value="/ListaNRecetaV.do"/>'>
...
<table class="formulario">
  ...
  <tr>
   ...
   <c:forEach var="listado" items="${listarKardex}" varStatus="contador">
     ...
     <form name=formaE<c:out value="${contador.count}"/> method=post action='<c:url value="/ListaNRecetaV.do"/>'>
       <td>     
    ...
    <td width="50%" valign="top">
     <form name=forma action="<c:url value="/ListaNRecetaV.do"/>" method="POST">        
         <table >
          <tr>    
  ...
  <tr>
   ...
   <c:forEach var="listado" items="${listarMedicamentos}" varStatus="contador">
     <!-- ********** Esto es para el efecto ************ -->
       <tr <c:if test="${(contador.count mod 2) == 0}">bgColor="#FFFFD9" %-- Est&acute;tico :( --%</c:if> onMouseOver="this.className='sobreFila'" onmouseout="this.className=''">
     <!-- ********** Fin  efecto ************ -->
     <form name=formaEm<c:out value="${contador.count}"/> method=post action='<c:url value="/ListaNRecetaV.do"/>'>
       <td><c:out value="${listado.medicamento}"/></td>      
       <td><c:out value="${listado.forma_far}"/></td>             
       <td style="font-size: 10pt"><b><c:out value="${listado.concentra}"/></b></td>      
       <td><c:out value="${listado.stockv}"/></td>      
       <td ><input type="text" name="precio" value='<c:out value="${listado.precio_venta}"/>' size=3 maxlength=6 /></td>                        
     
      <c:if test="${listado.stockv > 0}">
  ...
  <tr>
   ...
   <c:forEach var="listado" items="${listarMedicamentosCot}" varStatus="contador">
     <!-- ********** Esto es para el efecto ************ -->
       <tr <c:if test="${(contador.count mod 2) == 0}">bgColor="#FFFFD9" %-- Est&acute;tico :( --%</c:if> onMouseOver="this.className='sobreFila'" onmouseout="this.className=''">
     <!-- ********** Fin  efecto ************ -->
     <form name=formaEn<c:out value="${contador.count}"/> method=post action='<c:url value="/ListaNRecetaV.do"/>'>
       <td><c:out value="${listado.medicamento}"/></td>      
       <td><c:out value="${listado.forma_far}"/></td>             
       <td style="font-size: 10pt"><b><c:out value="${listado.concentra}"/></b></td>      
       <td><c:out value="${listado.stockv}"/></td>      
