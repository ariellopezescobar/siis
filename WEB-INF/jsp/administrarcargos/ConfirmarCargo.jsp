...
<form name="adicionarusu" method="POST" action='<c:url value="/GrabarCargo.do"/>' >
 <center>   
  <table class="table table-striped table-bordered table-condensed table-responsive">
    ...
    <c:if test="${sw != 1}">       
      <tr>
        ...
        <td><c:out value="${dato.id_cargo}"/></td>
      </tr>  
    </c:if>  
    <tr>
  ...
  <input type="hidden" name='id_cargo' value='<c:out value="${dato.id_cargo}"/>'>
  <input type="hidden" name='cargo' value='<c:out value="${dato.cargo}"/>'>
